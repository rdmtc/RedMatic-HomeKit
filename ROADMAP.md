...
2. **Role → HomeKit service** table: switch → Switch/Outlet/Lightbulb/Fan/
   ...
   HmIP-WRC2/WRC6/BRC2/KRC4/RC8/WRCC2, HM-PB-*, HM-RC-*), `LOWBAT` /
...
6. **CUxD**: CUxD answers `getParamsetDescription` (12 CUxD types cached
   ...
   their *own* channel layout, which fixes the wrong-datapoints class
...
- **Homebrew (HB-*) devices**: actuators/buttons from the JP-HB-Devices
  ...
  `SWITCH.STATE` on a `SIGNAL_LED` channel) → generic. Homebrew *sensors*
...
- One unreachable device makes *all* accessories "No Response" (#312,
