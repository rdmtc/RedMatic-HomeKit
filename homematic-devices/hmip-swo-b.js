module.exports = class HmipSwo {
    constructor(config, homematic) {
        ...
        const datapointIllumination = config.iface + '.' + config.description.ADDRESS + ':1.ILLUMINATION';
        const valueIllumination = (ccu.values && ccu.values[datapointIllumination] && ccu.values[datapointIllumination].value) || 0;
        ...
        const subtypeIllumination = '2';
        const subtypeBattery = '3';
        ...
        if (!acc.isConfigured) {
            ...
            acc.addService(hap.Service.LightSensor, config.name, subtypeIllumination)
                .getCharacteristic(hap.Characteristic.CurrentAmbientLightLevel)
                .setValue(valueIllumination);
        ...
        const getListenerIllumination = callback => {
            homematic.debug('get ' + config.name + ' ' + subtypeIllumination + ' CurrentAmbientLightLevel ' + getError() + ' ' + valueIllumination);
            callback(null, valueIllumination);
        };
        ...
        acc.getService(subtypeIllumination).getCharacteristic(hap.Characteristic.CurrentAmbientLightLevel).on('get', getListenerIllumination);
        ...
        const idSubscription = ccu.subscribe({
            ...
            switch (msg.channelIndex + '.' + msg.datapoint) {
                ...
                case '1.ILLUMINATION':
                    valueHumidity = msg.value;
                    homematic.debug('update ' + config.name + ' ' + subtypeIllumination + ' CurrentAmbientLightLevel ' + valueIllumination);
                    acc.getService(subtypeIllumination).updateCharacteristic(hap.Characteristic.CurrentAmbientLightLevel, valueIllumination);
                    break;
                default:
            }
        });
        ...
        homematic.on('close', () => {
            ...
            acc.getService(subtypeIllumination).getCharacteristic(hap.Characteristic.CurrentAmbientLightLevel).removeListener('get', getListenerIllumination);
