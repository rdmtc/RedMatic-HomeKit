module.exports = class HmipBdt {
    constructor(config, homematic) {
        ...
        const datapointBrightness = config.iface + '.' + config.description.ADDRESS + ':3.LEVEL';
        ...
        const setListenerBrightness = (value, callback) => {
            ...
            ccu.setValue(config.iface, config.description.ADDRESS + ':3', 'LEVEL', value / 100)
                .then(() => {
                    callback();
                })
                .catch(() => {
        ...
        const setListenerOn = (value, callback) => {
            ...
            if (valueBrightness === 0 || !value) {
                ccu.setValue(config.iface, config.description.ADDRESS + ':3', 'LEVEL', value ? 1 : 0)
                    .then(() => {
                        callback();
                    })
                    .catch(() => {
