module.exports = class HmipSmi55 {
    constructor(config, homematic) {
        ...
        const datapointMotion = config.iface + '.' + config.description.ADDRESS + ':3.MOTION';
        let valueMotion = Boolean(ccu.values && ccu.values[datapointMotion] && ccu.values[datapointMotion].value);

        const datapointBrightness = config.iface + '.' + config.description.ADDRESS + ':3.ILLUMINATION';
