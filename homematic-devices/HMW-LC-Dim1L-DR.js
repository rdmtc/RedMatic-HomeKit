    ...
    constructor(config, iface) {
        ...
        acc.addService(hap.Service.Lightbulb, config.name, '0')
            ...
            .on('set', (value, callback) => {
                ...
                if (value && !currentLevel) {
                    iface.emit('setValue', {address: config.description.ADDRESS + ':3', datapoint: 'LEVEL', value: 100});
                } else if (!value) {
                    iface.emit('setValue', {address: config.description.ADDRESS + ':3', datapoint: 'LEVEL', value: 0});
                }
                callback();
            });
        ...
        acc.getService('0')
            ...
            .on('set', (value, callback) => {
                ...
                iface.emit('setValue', {
                    address: config.description.ADDRESS + ':3',
                    datapoint: 'LEVEL',
                    value: value / 100
                });
                callback();
