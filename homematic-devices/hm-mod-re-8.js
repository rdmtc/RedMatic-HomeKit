module.exports = class HmModRe8 {
    constructor(config, homematic) {
        ...
        const name5 = ccu.channelNames[config.description.ADDRESS + ':5'];
        const name6 = ccu.channelNames[config.description.ADDRESS + ':6'];
        const name7 = ccu.channelNames[config.description.ADDRESS + ':7'];
        const name8 = ccu.channelNames[config.description.ADDRESS + ':8'];
        ...
        const datapointOn5 = config.iface + '.' + config.description.ADDRESS + ':5.STATE';
        let valueOn5 = ccu.values && ccu.values[datapointOn5] && ccu.values[datapointOn5].value;

        const datapointOn6 = config.iface + '.' + config.description.ADDRESS + ':6.STATE';
        let valueOn6 = ccu.values && ccu.values[datapointOn6] && ccu.values[datapointOn6].value;

        const datapointOn7 = config.iface + '.' + config.description.ADDRESS + ':7.STATE';
        let valueOn7 = ccu.values && ccu.values[datapointOn7] && ccu.values[datapointOn7].value;

        const datapointOn8 = config.iface + '.' + config.description.ADDRESS + ':8.STATE';
        let valueOn8 = ccu.values && ccu.values[datapointOn8] && ccu.values[datapointOn8].value;
        ...
        if (!acc.isConfigured) {
            ...
            acc.addService(hap.Service.Switch, name5, subtype5)
                .updateCharacteristic(hap.Characteristic.On, valueOn5);

            acc.addService(hap.Service.Switch, name6, subtype6)
                .updateCharacteristic(hap.Characteristic.On, valueOn6);

            acc.addService(hap.Service.Switch, name7, subtype7)
                .updateCharacteristic(hap.Characteristic.On, valueOn7);

            acc.addService(hap.Service.Switch, name8, subtype8)
                .updateCharacteristic(hap.Characteristic.On, valueOn8);
        ...
        const setListener5 = (value, callback) => {
            homematic.debug('set ' + config.name + ' ' + subtype5 + ' On ' + value);
            ccu.setValue(config.iface, config.description.ADDRESS + ':5', 'STATE', value)
                .then(() => {
                    callback();
                })
                .catch(() => {
        ...
        const setListener6 = (value, callback) => {
            homematic.debug('set ' + config.name + ' ' + subtype6 + ' On ' + value);
            ccu.setValue(config.iface, config.description.ADDRESS + ':6', 'STATE', value)
                .then(() => {
                    callback();
                })
                .catch(() => {
        ...
        const setListener7 = (value, callback) => {
            homematic.debug('set ' + config.name + ' ' + subtype7 + ' On ' + value);
            ccu.setValue(config.iface, config.description.ADDRESS + ':7', 'STATE', value)
                .then(() => {
                    callback();
                })
                .catch(() => {
        ...
        const setListener8 = (value, callback) => {
            homematic.debug('set ' + config.name + ' ' + subtype8 + ' On ' + value);
            ccu.setValue(config.iface, config.description.ADDRESS + ':8', 'STATE', value)
                .then(() => {
                    callback();
                })
                .catch(() => {
        ...
        const getListener5 = callback => {
            homematic.debug('get ' + config.name + ' ' + subtype5 + ' On ' + getError() + ' ' + valueOn5);
            callback(getError(), valueOn5);
        };

        const getListener6 = callback => {
            homematic.debug('get ' + config.name + ' ' + subtype6 + ' On ' + getError() + ' ' + valueOn6);
            callback(getError(), valueOn6);
        };

        const getListener7 = callback => {
            homematic.debug('get ' + config.name + ' ' + subtype7 + ' On ' + getError() + ' ' + valueOn7);
            callback(getError(), valueOn7);
        };

        const getListener8 = callback => {
            homematic.debug('get ' + config.name + ' ' + subtype8 + ' On ' + getError() + ' ' + valueOn8);
            callback(getError(), valueOn8);
        };
        ...
        acc.getService(subtype5).getCharacteristic(hap.Characteristic.On).on('get', getListener5);
        acc.getService(subtype6).getCharacteristic(hap.Characteristic.On).on('get', getListener6);
        acc.getService(subtype7).getCharacteristic(hap.Characteristic.On).on('get', getListener7);
        acc.getService(subtype8).getCharacteristic(hap.Characteristic.On).on('get', getListener8);
        ...
        acc.getService(subtype5).getCharacteristic(hap.Characteristic.On).on('set', setListener5);
        acc.getService(subtype6).getCharacteristic(hap.Characteristic.On).on('set', setListener6);
        acc.getService(subtype7).getCharacteristic(hap.Characteristic.On).on('set', setListener7);
        acc.getService(subtype8).getCharacteristic(hap.Characteristic.On).on('set', setListener8);
        ...
        const idSubscription = ccu.subscribe({
            ...
            switch (msg.channelIndex + '.' + msg.datapoint) {
                ...
                case '5.STATE':
                    valueOn5 = msg.value;
                    homematic.debug('update ' + config.name + ' ' + subtype5 + ' On ' + valueOn5);
                    acc.getService(subtype5).updateCharacteristic(hap.Characteristic.On, valueOn5);
                    break;
                case '6.STATE':
                    valueOn6 = msg.value;
                    homematic.debug('update ' + config.name + ' ' + subtype6 + ' On ' + valueOn6);
                    acc.getService(subtype6).updateCharacteristic(hap.Characteristic.On, valueOn6);
                    break;
                case '7.STATE':
                    valueOn7 = msg.value;
                    homematic.debug('update ' + config.name + ' ' + subtype7 + ' On ' + valueOn7);
                    acc.getService(subtype7).updateCharacteristic(hap.Characteristic.On, valueOn7);
                    break;
                case '8.STATE':
                    valueOn8 = msg.value;
                    homematic.debug('update ' + config.name + ' ' + subtype8 + ' On ' + valueOn8);
                    acc.getService(subtype8).updateCharacteristic(hap.Characteristic.On, valueOn8);
                    break;
                default:
            }
        });
        ...
        homematic.on('close', () => {
            ...
            acc.getService(subtype5).getCharacteristic(hap.Characteristic.On).removeListener('get', getListener5);
            acc.getService(subtype6).getCharacteristic(hap.Characteristic.On).removeListener('get', getListener6);
            acc.getService(subtype7).getCharacteristic(hap.Characteristic.On).removeListener('get', getListener7);
            acc.getService(subtype8).getCharacteristic(hap.Characteristic.On).removeListener('get', getListener8);
            ...
            acc.getService(subtype5).getCharacteristic(hap.Characteristic.On).removeListener('set', setListener5);
            acc.getService(subtype6).getCharacteristic(hap.Characteristic.On).removeListener('set', setListener6);
            acc.getService(subtype7).getCharacteristic(hap.Characteristic.On).removeListener('set', setListener7);
            acc.getService(subtype8).getCharacteristic(hap.Characteristic.On).removeListener('set', setListener8);
        });
    }
};
